...
# for KVIK25 and onwards we need to exclude strøm used. This was by accident included in KVIK25
...
emis_summary <- emis[, .(CH4_dyr_stald = unique(CH4_dyr_stald), 
                         ...
                         strøm_CO2_ton = unique(strøm_CO2_ton),
...
out <- out[, ":="(reduktion_totCO2_eq_tot = (totCO2_eq_tot[Scenarie == 'kontrol'] - totCO2_eq_tot) * ((potentiale-udbredelse)/100),
                  reduktion_CO2_eq_tot_m3 = CO2_eq_tot[Scenarie == 'kontrol'] - CO2_eq_tot,
                  reduktion_N2O_tot_m3 = N2O_dyr_tot[Scenarie == 'kontrol'] - N2O_dyr_tot,
                  reduktion_CH4_tot_m3 = CH4_dyr_tot[Scenarie == 'kontrol'] - CH4_dyr_tot,
                  reduktion_N2O_CO2eq_tot_m3 = (N2O_dyr_tot[Scenarie == 'kontrol'] - N2O_dyr_tot) * CO2_eq[['N2O']],
                  reduktion_CH4_CO2eq_tot_m3 = (CH4_dyr_tot[Scenarie == 'kontrol'] - CH4_dyr_tot) *  CO2_eq[['CH4']],
                  reduktion_strøm_CO2eq_tot_m3 = (strøm_CO2_ton[Scenarie == 'kontrol']- strøm_CO2_ton)), by = 'model_gruppe'][order(Scenarie)]
...
out_dyr <- out_dyr[, .(sum_reduktion_totCO2_eq_tot = sum(reduktion_totCO2_eq_tot),
            ...
            mean_reduktion_CH4_CO2_eq_tot_m3 = sum(reduktion_CH4_CO2eq_tot_m3 * TotGoednabDyr_kt_year)/sum(TotGoednabDyr_kt_year),
            mean_reduktion_N2O_CO2_eq_tot_m3 = sum(reduktion_N2O_CO2eq_tot_m3 * TotGoednabDyr_kt_year)/sum(TotGoednabDyr_kt_year),
            mean_reduktion_strøm_CO2_eq_tot_m3 = sum(reduktion_strøm_CO2eq_tot_m3 * TotGoednabDyr_kt_year)/sum(TotGoednabDyr_kt_year),
...
cols_to_remove <- c(
  'reduktion_N2O_tot_m3',
  'reduktion_CH4_tot_m3',
  'reduktion_N2O_CO2eq_tot_m3',
  'reduktion_CH4_CO2eq_tot_m3',
  'reduktion_strøm_CO2eq_tot_m3',
  'strøm_CO2_ton'
)

out <- out[, setdiff(names(out), cols_to_remove), with = FALSE]
